...
@Value
public class EventCondition implements Comparable<EventCondition> {
    @NonNull String event;
    @NonNull String conditionName;

    @Override
    public int compareTo(EventCondition o) {
        int result = this.event.compareTo(o.event);
        if (result == 0) {
            return this.conditionName.compareTo(o.conditionName);
        }
        return result;
    }
}
